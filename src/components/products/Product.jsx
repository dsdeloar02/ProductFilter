...
import { getProductData } from '../../data/TamplateData';
...
const Product = () => {
    ...
    return (
        <div className="product-area-full ">
            <h2 className='mt-5 text-2xl font-bold text-red-900'>Products</h2>
            <br />
            <ProductFilter />
            <div className="product-area">
                {
                    products.map((product, index) => (
                        <div key={index} className="product-single">
                            <p>
                                <img className='h-[150px] w-auto flex mx-auto' src={product.image}  alt="" />
                            </p>
                            <h3 className='py-2 '>{product.title}</h3>
                            <h3 className='text-red-600 py-2' >{product.price} BDT</h3>
                            <p className='my-2'>
                                <Link to={`/products/${product.id}`} className="bg-black  text-white py-2 px-4 hover:bg-red-400 text-white">
                                    Products Details
                                </Link>
                            </p>
